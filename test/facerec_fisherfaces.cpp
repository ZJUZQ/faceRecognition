...
int main( int argc, char** argv ){
    ...
    cv::Ptr<FACE::BasicFaceRecognizer> model = FACE::createFisherFaceRecognizer(); // will use full PCA
    ...
    // Display or save the fisherfaces, at most 16 Fisherfaces:
    for( int i = 0; i < std::min( 16, V.cols ); i++ ){
    ...
    for( int num_component = 0; num_component < std::min( V.cols, 16 ); num_component++ ){
        // slice the eigenvectors from the model
        cv::Mat ev = V.col( num_component );
        cv::Mat projection = cv::LDA::subspaceProject( ev, mean, images[0].reshape( 1, 1 ) );
