...
int main( int argc, const char** argv ){
    ...
    // The following lines simply get the last images from your dataset and remove it from the vector. This is
    // done, so that the training data (which we learn the cv::LBPHFaceRecognizer on) and the test data we test
    ...
    //
    //      cv::createLBPHFaceRecognizer(2, 16); 指定半径R和邻点数目P
    //
    // And if you want a threshold (e.g. 123.0) call it with its default values:
    //
    //      cv::createLBPHFaceRecognizer( 1, 8, 8, 8, 123.0 )
    //
    cv::Ptr<FACE::LBPHFaceRecognizer> model = FACE::createLBPHFaceRecognizer();
